...
An example [docker-compose.yml](docker-compose.yml) file is provided that builds and deploys 
...
An example [docker-compose.yml](docker-compose.yml) file is provided that builds and deploys (`-d` is used to daemonize the call).
...
Use the `docker exec` call to gain a shell on either the `worker01` or `worker02`or `worker03` or `worker04` container and become the user `worker`.
...
- **Storage Host:** The MySQL database server is located at "database.local.dev."
- **Storage Port:** Communication with the MySQL server is on port 3306.
- **Storage Password:** The MySQL user "slurm" authenticates with the password "password."
- **Storage User:** SlurmDBD connects to the MySQL server as the user "slurm."
- **Storage Location:** Accounting and job data are stored in the database named "slurm_acct_db."
